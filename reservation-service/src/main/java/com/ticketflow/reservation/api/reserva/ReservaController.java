 ...
 *
 * <p>Expone el endpoint {@code POST /api/v1/reservas/bloquear} que
 * permite a un cliente bloquear un asiento disponible durante 5
 * minutos (TTL) mientras completa el flujo de compra. El bloqueo se
 * materializa como una clave en Redis con formato
 * {@code lock:funcion:{funcionId}:asiento:{asientoId}}; no se crea
 * ninguna fila en la base de datos en esta fase.</p>
 *
 * <p>El endpoint valida que:</p>
 * <ul>
 ...
 * </ul>
 *
 * <p>En caso de éxito, devuelve un {@code reservationId} (token
 * opaco) que el cliente debe conservar para confirmar la compra
